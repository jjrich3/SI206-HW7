...
def birthyear_nationality_search(age, country, cur, conn):
    players = []
    age_year = 2023 - age
    cur.execute("SELECT name, nationality, birthyear FROM Players WHERE nationality=(?) AND birthyear<(?)",(country,age_year))
    for player in cur:
        players.append(player)
    return players     
    pass
...
def position_birth_search(position, age, cur, conn):
    ...
    for pos in range(len(positions)):
        if position == positions[pos][1]:
            pos_id = positions[pos][0]
    players = []
    age_year = 2023 - age
    cur.execute("SELECT name, position_id, birthyear FROM Players WHERE position_id=(?) AND birthyear>(?)",(pos_id,age_year))
    for player in cur:
        players.append((player[0],position,player[2]))
    return players     
    pass
...
def make_seasons_table(data, cur, conn):
    ...
    for i in range(len(seasons)):
        cur.execute("INSERT OR IGNORE INTO Seasons (id, winner_id, end_year) VALUES (?,?,?)",(seasons[i][0], seasons[i][1], seasons[i][2]))
    conn.commit()
    pass

def winners_since_search(year, cur, conn):
    winners = {}
    cur.execute("SELECT winner_id FROM Seasons WHERE end_year>(?)",(int(year),))
    winner_ids = {}
    for team in cur:
        winner_ids[team[0]]= winner_ids.get(team[0],0)+1
    #print(winner_ids)
    ids = list(winner_ids.keys())
    for id in ids:
        cur.execute("SELECT name FROM Winners WHERE id=(?)",(int(id),))
        for name in cur:
            print(name)
            winners[name[0]] = winner_ids[id]
        #print(winners[name])
    #print(winners)
    return winners     
    pass
...
class TestAllMethods(unittest.TestCase):
    ...
    def test_winners_since_search(self):
        a = winners_since_search("2000",self.cur2,self.conn2)
        self.assertEqual(a['Manchester City FC'],5)
        self.assertEqual(a['Liverpool FC'],1)
        pass
